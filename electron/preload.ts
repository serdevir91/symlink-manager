...
contextBridge.exposeInMainWorld('symlink', {
    create: (linkPath: string, targetPath: string, type: 'file' | 'dir') =>
        ipcRenderer.invoke('symlink:create', linkPath, targetPath, type),
    remove: (linkPath: string) =>
        ipcRenderer.invoke('symlink:remove', linkPath),
    scan: (directoryPath: string) =>
        ipcRenderer.invoke('symlink:scan', directoryPath),
    validate: (linkPath: string) =>
        ipcRenderer.invoke('symlink:validate', linkPath),
    info: (linkPath: string) =>
        ipcRenderer.invoke('symlink:info', linkPath)
} as SymlinkAPI);
