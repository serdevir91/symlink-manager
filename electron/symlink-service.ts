...
): Promise<{ success: boolean; symlinks?: SymlinkInfo[]; error?: string }> {
    try {
...
export async function getSymlinkInfo(
    linkPath: string
): Promise<{ success: boolean; info?: SymlinkInfo; error?: string }> {
    try {
        const stats = await lstat(linkPath);
 ...
 */
async function checkTargetExists(targetPath: string, basePath: string): Promise<boolean> {
    try {
